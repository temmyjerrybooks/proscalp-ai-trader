...
- **Live smoke test:** [scripts/algo_order_smoke.py](scripts/algo_order_smoke.py) — run inside the container against demo-fapi; validates the full lifecycle for all three types. **NEW staging-gate requirement (see below).**

**The two-flip activation sequence below is paused and resumes only after this adapter fix passes its staging gate, merges, and deploys.** Both Branch 2 flags remain `False`; the ladder code is otherwise complete and inert.

### Pending Activation Sequence (Branch 2) — agreed 2026-05-29, **PAUSED 2026-05-30 pending adapter fix**

Deployed gated (both new flags default False) on 2026-05-29; **first flip attempted + rolled back 2026-05-30 (see above).** Activation is a **deliberate, staged, operator-triggered** sequence — never flip flags without an explicit operator request in-session:
