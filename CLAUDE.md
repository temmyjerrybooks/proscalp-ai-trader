...
## ⚑ CURRENT DEPLOYED STATE (2026-06-06, SIGNAL-ENGINE FRAMEWORK DEPLOYED INERT; EXIT SAGA CLOSED) — AUTHORITATIVE

This block supersedes every older "exit-fix inert / 873f30b / 120-run / armed / reverted-to-single-TP" summary anywhere below — those are history.

**Prod runs `phase-2c-signal-engines`, commit `071f18b` (built on the validated exit-fix `2eb2d49`), deployed INERT — `signal_engine_mode=classic`, loop stopped, flat. NOT trading.**
- Backend **and** frontend containers rebuilt 2026-06-06 (`docker compose up -d --no-deps --build`; postgres/nginx untouched). In-container hashes verified == `071f18b` (bot_runner `98157a4d…`, settings `c7b6b660…`, stat_arb `e58ee3d7…`). Backend **healthy**. Predeploy backups: `app.bak-pre-071f18b` (backend) / `src.bak-pre-071f18b` (frontend). Staging gate: full suite **234 passed** in the sealed prod image. No `binance_adapter` change → live algo-smoke gate not triggered. Branch pushed to the private remote.

- **EXIT SAGA CLOSED — attribution VALIDATED.** The recurring over/under-book bug is dead. Exit attribution is now **userTrades-authoritative** (`GET /fapi/v1/userTrades` is the settled-fill ledger; conservation law `base == exit_qty + position_qty`) — commit `2eb2d49`, which **REPLACED** the earlier `873f30b` qty-drop gate (that under-booked). Root cause was inferring fills from the eventually-consistent algo-status surface (`-2013` swallowed as "resting"). **Re-verified live against the prod DB: 15 ladder closes since T0 2026-06-05 19:06Z → 0 `ladder_sync_anomaly`, 0 `ladder_false_fill_rejected`, 0 breakers; R:R 1.28** (avg win $2.46 vs avg loss $1.92; pre-fix was 0.45). Winners bank real tier/runner gradients (ZEC +4.94, XMR +1.88). **That cohort was net −$21 / ~17% win — that is the SIGNAL/direction bleed, NOT exits.** Do not re-litigate exit attribution; read userTrades, never algo status. (TASK 2 TP recalibration `tp_tier_atr_multipliers=[0.6,1.0,1.5,2.0]` + TASK 3 rich exit alert from `873f30b` remain live.)

- **SIGNAL-ENGINE FRAMEWORK (the response to the signal bleed).** New `app/signal_engines/` — a pluggable `SignalEngine` layer so strategy families are A/B-tested on testnet. Risk sizing, execution, and the userTrades exit ladder are **SHARED/engine-agnostic**; only entry-signal generation differs. Selected by `signal_engine_mode` (default `classic`); switchable at runtime via the Dashboard "Signal Engine" dropdown + `GET/POST /api/bot/signal-engine[s]`. Every trade is stamped `extra["signal_engine"]` → per-engine scoreboard.
  - **`classic`** — thin pass-through to the existing 7-setup `_scan_for_signals`; **byte-for-byte the pre-framework behavior**; the committed default; INERT.
  - **`stat_arb`** — single-leg BTC-relative mean reversion (z-score of `ln(alt/BTC)`; fade when stretched + reverting; pure core `stat_arb_core.py`; tunable `stat_arb_*`: `entry_z=2.0`, `lookback=60`, `stop_atr_mult=1.2`). **DEPLOYED but NEVER run live.**

- **Flags still INERT (`.env` safe defaults):** `five_tier_ladder_enabled=False`, `exchange_resting_exits_enabled=False`, `allow_unclear_regime_trading=False`. **Loop STOPPED.** Flat 0/0. `consecutive_loss_stop_after=4`. Phase-2A gates intact (`cap_risk_at_score=75`, `force_limit_orders=True`, `max_concurrent_trades=5`). `.env` still carries `BOT_MAX_ORDERS_PER_CYCLE=2` / `BOT_MIN_SECONDS_BETWEEN_ORDERS=10` (inert while stopped).

- **NEXT STEP — LIVE-TEST `stat_arb` (separate, gated, WATCHED, not done).** First real swing at the ~17% signal bleed. Clean-test recipe: arm the validated exit path (`exchange_resting_exits_enabled` then `five_tier_ladder_enabled`) so exits don't contaminate the comparison, switch engine to `stat_arb`, LOW concurrency (`bot_max_orders_per_cycle=1`), `/api/bot/start`, fresh T0, watch the per-engine scoreboard. **Never leave it armed-and-unwatched.** Remaining brainstormed engines not built: Forced-Flow (liquidation/funding/OI), LLM-Catalyst.
...
Last updated: 2026-06-06 — Exit saga CLOSED: userTrades-authoritative attribution (`2eb2d49`) validated live (15 closes, 0 anomalies, R:R 1.28); the bleed is now the SIGNAL/direction layer (~17% win). Built + deployed the pluggable signal-engine framework + a single-leg BTC-relative Stat-Arb engine (`phase-2c-signal-engines`, `071f18b`) INERT (`classic` default), switchable via UI/API; `stat_arb` never run live. See top "CURRENT DEPLOYED STATE" block.
