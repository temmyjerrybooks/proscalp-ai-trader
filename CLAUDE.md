...
**Branch 2 — `phase-2b-exits`:** IMPLEMENTED 2026-05-29, **NOT DEPLOYED** (commit `081ab43`, on top of config slice `66d23b5`). BE+ stop, 5-tier TP ladder (4×20% tiers + 20% trailing runner), stop progression (BE+ → +0.2% → +0.5% → +1.0%), trailing runner, time-based exits (partial 15min / full 45min), slippage anomaly logging. Signals/regime/volume are explicitly OUT (Phase 2C). Full suite 149 passing; all trading-impact gated OFF.
...
- **Day +2–3 (after resting-orders proves clean):** operator flips **`five_tier_ladder_enabled = True` AND `allow_unclear_regime_trading = True` together**. This starts the **120-trade judged clock** (note: `allow_unclear_regime_trading` is a separate behavioral lever — it widens trading to unclear regimes — bundled into this step by operator direction).
