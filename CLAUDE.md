...
**Next planned work — `phase-2b-adapter-algo-fix` (investigation-first):** route `STOP_MARKET` / `TAKE_PROFIT_MARKET` / `TRAILING_STOP_MARKET` to the Algo Order API (`/fapi/v1/algo/futures/...`); determine whether the hand-rolled `BinanceAdapter` needs explicit endpoint switching for conditional types (or whether a ccxt path auto-routes them); validate against the **live testnet** (not just docs/mocks) before re-attempting activation. **The two-flip activation sequence below is paused and resumes only after this adapter fix is built, tested, and deployed.** Both Branch 2 flags remain `False`; the ladder code is otherwise complete and inert.
